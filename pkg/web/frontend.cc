...
list_resources(std::optional<std::filesystem::path> ui_folder) {
  if (not ui_folder) {
    return {};
  }
...
Frontend::Frontend(std::optional<std::filesystem::path> ui_folder)
    : _ui_folder{ui_folder.value_or("./ui")}, _endpoint{
                                                  Address{Ipv4::any(), Port}} {}
...
void Frontend::shutdown() { _endpoint.shutdown(); }
